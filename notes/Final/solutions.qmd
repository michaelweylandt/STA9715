...
We recognize this as the PDF of a $\text{Beta}(2, 5)$ distribution, hence it has $$\E[X] = \frac{\alpha}{\alpha + \beta} = \frac{2}{7} \approx 28.57\%$$
:::
...
2) If we do not recognize this as a common MGF, we can differentiate through 
   the MGF to get the first and second central moments.
    ...
    Hence, $$\V[X] = \E[X]^2 - \E[X]^2 = M''_X(0) - M'_X(0)^2 = 41 - 5^2 = 16$$ which gives a standard deviation of 4 for $X$. $X$
:::
...
they a D or an F in the course?
...
data.frame(A1 = sample(1:6, n, replace=TRUE),
    ...
    mutate(A = pmax(A1, A2)) |>
    mutate(B_wins = B >= A)  |>
    summarize(P_B_wins = mean(B_wins))
```
...
Since here $\Var[X] \propto 1/n$, we can instead write this as: 
...
= 6^2 - 2 * 6 * 6 * \frac{1}{4} + 6^2 = 54$$

Hence, standardization of $S - F$ gives us:

$$\P(M > D + 4) = 1 - F_{M-D}(4) = 1 - \Phi\left(\frac{4 - 0}{\sqrt{54}}\right) \approx 29.3\%$$
...
$$\V[(M, D)] = \begin{pmatrix} 6^2 & 6 * 6 * \frac{1}{4} \\ 6 * 6 * \frac{1}{4} & 6^2 \end{pmatrix} = \begin{pmatrix} 36 & 9 \\ 9 & 36 \end{pmatrix}$$ Next, we use `MASS::mvrnorm` to generate our samples:

```{r echo=TRUE}
library(MASS)
mu <- c(72, 72)
Sigma <- matrix(c(36, 9, 9, 36), nrow=2)
